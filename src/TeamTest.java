...
public class TeamTest {
	public static void main(String[] args) throws FileNotFoundException {
		...
		for(int i = 0; i < 300; i++) list.add(new Team(i));
		System.out.println(list);
		System.out.println(Team.getTeam(list, "Poop"));
	}
}
/*
import java.util.ArrayList;
...
public class Tournament {
	...
	public Tournament(ArrayList<Team> teams) {
		...
		for(int i = 0; i < this.teams.size(); i++) {
			this.teams.get(i).setOpponent(this.teams.get(16*(1 + i/16) - i%16 - 1));
			if(!this.teams.get(i).getPlayed()) {
				this.teams.get(i).setIndex(count);
				this.teams.get(i).getOpponent().setIndex(count);
